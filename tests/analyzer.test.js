...
describe("analyzer", () => {
  ...
  describe("analyzeHead", () => {
    ...
    it("should include validation warnings by default", () => {
      ...
      const titleWarning = result.validationWarnings.find((w) => w.warning.includes("<title>"));
    ...
    it("should include custom validations by default", () => {
      ...
      const charsetValidation = result.customValidations.find((v) => v.warnings.some((w) => w.includes("UTF-8")));
  ...
  describe("checkOrdering", () => {
    it("should detect ordering violations", () => {
      ...
      assert.ok(
        violations.some((v) => v.nextCategory === "TITLE"),
        "Should detect title violation",
      );
      assert.ok(
        violations.some((v) => v.nextCategory === "META"),
        "Should detect meta violation",
      );
    });
