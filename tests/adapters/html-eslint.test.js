...
describe('HtmlEslintAdapter', () => {
  ...
  describe('matches', () => {
    it('should match simple tag selector', () => {
      setup();
      const node = createNode('meta', [['charset', 'utf-8']]);
      assert.equal(adapter.matches(node, 'meta'), true);
      assert.equal(adapter.matches(node, 'link'), false);
    });

    it('should match attribute selector', () => {
      setup();
      const node = createNode('meta', [['charset', 'utf-8']]);
      assert.equal(adapter.matches(node, 'meta[charset]'), true);
      assert.equal(adapter.matches(node, 'meta[name]'), false);
    });

    it('should match attribute value selector', () => {
      setup();
      const node = createNode('meta', [['charset', 'utf-8']]);
      assert.equal(adapter.matches(node, 'meta[charset="utf-8"]'), true);
      assert.equal(adapter.matches(node, 'meta[charset="iso-8859-1"]'), false);
    });

    it('should match case-insensitive attribute value selector', () => {
      setup();
      const node = createNode('meta', [['charset', 'UTF-8']]);
      assert.equal(adapter.matches(node, 'meta[charset="utf-8" i]'), true);
      assert.equal(adapter.matches(node, 'meta[charset="UTF-8" i]'), true);
    });
    ...
    it('should match complex selector', () => {
      ...
      assert.equal(adapter.matches(node, 'link[rel="stylesheet"]'), true);
      assert.equal(adapter.matches(node, 'link[rel="preload"]'), false);
    });

    it('should match wildcard selector', () => {
      setup();
      const node = createNode('meta', [['charset', 'utf-8']]);
      assert.equal(adapter.matches(node, '*'), true);
    });

    it('should match selector with tag and multiple attributes', () => {
      setup();
      const node = createNode('meta', [
        ['name', 'viewport'],
        ['content', 'width=device-width']
      ]);
      // Note: Current implementation doesn't support multiple attribute selectors
      // Just test that it matches each individually
      assert.equal(adapter.matches(node, 'meta[name="viewport"]'), true);
      assert.equal(adapter.matches(node, 'meta[content]'), true);
    });

    it('should return false for invalid selector', () => {
      setup();
      const node = createNode('meta', [['charset', 'utf-8']]);
      // matchesSelector helper will return false for unsupported selectors
      assert.equal(adapter.matches(node, 'meta > link'), false);
    });
