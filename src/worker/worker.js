...
async function handleRequest(request, env) {
  ...
  if (pathname === "/auth") {
    ...
    try {
      ...
      if (client_id === env.DEV_GITHUB_CLIENT_ID) {
        targetClientId = env.DEV_GITHUB_CLIENT_ID;
        targetClientSecret = env.DEV_GITHUB_CLIENT_SECRET;
      } else if (client_id === env.PROD_GITHUB_CLIENT_ID) {
        targetClientId = env.PROD_GITHUB_CLIENT_ID;
      ...
      return new CorsResponse(origin, JSON.stringify({ error: err instanceof Error ? err.message : String(err) }), {
        status: 500,
        headers: { "Content-Type": "application/json" },
      });
    }
