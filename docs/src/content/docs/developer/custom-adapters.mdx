...
// 3. Analyze (optionally pass pageOrigin for Origin Trial validation in non-browser/CLI environments)
const results = analyzeHead(head, adapter, { pageOrigin: 'https://example.com' });
