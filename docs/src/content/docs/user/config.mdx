...
The extension supports [custom color palettes](#custom-color-palette).
...
When using capo.js via a script or the npm package, you can configure it by setting a `window.__CAPO__` object before the script runs.
...
You can provide colors in any format supported by your browser, for example: hex, rgb, hsl, oklch, named colors, etc. Formats can also be mixed and matched within the same config.
