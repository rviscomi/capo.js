...
## <InlineCapoWeightBars weight="9" /> 9. Critical resource hints
...
High-priority preload hints (`<link rel="preload" fetchpriority="high">` or `<link rel="modulepreload" fetchpriority="high">`) tell the browser to download a critical resource (such as the LCP image or critical font) with maximum urgency.

By elevating these preloads alongside preconnect hints, capo.js ensures that the browser's Preload Scanner discovers them in the initial ~14 KB TCP window (`initcwnd`), even on pages with extensive inline styles. Standard preloads without `fetchpriority="high"` remain in [**4. Standard preload hints**](#link-relpreload).
...
:::note
Stylesheets explicitly marked for print media (e.g. `<link rel="stylesheet" media="print">` or `<style media="print">`) are not render-blocking for screen media. capo.js does not classify them as synchronous styles and instead gives them the lowest weight (**1. Everything else**).
:::
...
While helpful, standard preloads (without `fetchpriority="high"`) are generally less critical than actual resources (like styles and scripts) that are needed immediately. capo.js recommends placing them after critical resources, but before deferred scripts and prefetch hints, to ensure these resources start downloading as soon as possible without competing for bandwidth against critical stylesheets.

Preloads explicitly configured with `fetchpriority="high"` are elevated to [**9. Critical resource hints**](#link-relpreload-fetchpriorityhigh).
