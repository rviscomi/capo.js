...
When evaluating HTML programmatically or in non-browser environments (e.g. linters or external URL fetchers), pass the `pageOrigin` option to `analyzeHead(headNode, adapter, { pageOrigin: 'https://example.com' })` so origin trial tokens are validated against the target page origin instead of `document.location`.
:::
...
In the example above, capo.js warns that there is an "Invalid origin trial token". The warning also includes a reference to the `<meta>` element as well as the decoded token metadata.
