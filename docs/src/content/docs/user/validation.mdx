...
Additionally, some CSP directives are not allowed in the `<meta>` declaration: `frame-ancestors`, `sandbox`, and `report-uri`. If capo.js detects any of these directives in a `<meta>` CSP element, it will log a validation warning.
