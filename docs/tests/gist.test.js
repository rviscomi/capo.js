...
describe('gist library', () => {
  ...
  describe('fetchGist', () => {
    ...
    it('should fetch raw_url if content is truncated', async () => {
      const mockApiResponse = {
        ...
        json: async () => ({
          files: {
            'index.html': { filename: 'index.html', truncated: true, raw_url: 'https://raw.githubusercontent.com/gists/raw-file' }
          }
        })
      };
  ...
  describe('exchangeCodeForToken', () => {
    it('should exchange code for access token successfully', async () => {
      ...
      const token = await exchangeCodeForToken('auth-code', 'http://localhost/redirect', 'mock-client-id', 'https://proxy/auth');
      expect(token).toBe('mock-access-token');
      expect(globalThis.fetch).toHaveBeenCalledWith('https://proxy/auth', expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({ code: 'auth-code', redirect_uri: 'http://localhost/redirect', client_id: 'mock-client-id' })
      }));
    });
    ...
    it('should throw error if server returns error info', async () => {
      ...
      await expect(exchangeCodeForToken('auth-code', 'http://localhost/redirect', 'mock-client-id', 'https://proxy/auth')).rejects.toThrow('The code passed is incorrect');
    });
  });
  ...
  describe('createGist', () => {
    it('should successfully create a gist and return the id', async () => {
      ...
      expect(globalThis.fetch).toHaveBeenCalledWith('https://api.github.com/gists', expect.objectContaining({
        method: 'POST',
        headers: {
